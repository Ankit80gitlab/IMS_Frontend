...
describe('TokenInterceptor', () => {
  ...
  it('should append token when url does not has http scheme', () => {
    ...
    const headers = mockRequest('/userInfo', user).request.headers;
  ...
  it('should append token when url include base url', () => {
    ...
    const headers = mockRequest(`${baseUrl}/userInfo`, user).request.headers;
  ...
  it('should clear token when response status is unauthorized', () => {
    ...
    mockRequest('/userInfo', {}, { status: STATUS.UNAUTHORIZED, statusText: 'Unauthorized' });

    expect(tokenService.clear).toHaveBeenCalled();
  });

  it('should navigate /login when api url is /logout and token is valid', () => {
    init('', 'token');
  ...
  it('should navigate /login when api url is /logout and token is invalid', () => {
    init('', '');
    const navigateByUrl = spyOn(router, 'navigateByUrl');
    navigateByUrl.and.returnValue(Promise.resolve(true));
