...
describe('BaseUrlInterceptor', () => {
  ...
  it('should not prepend base url when base url is empty', () => {
    ...
    http.get('/userInfo').subscribe(data => expect(data).toEqual({ success: true }));

    httpMock.expectOne('/userInfo').flush({ success: true });
  });
  ...
  it('should prepend base url when request url does not has http scheme', () => {
    ...
    http.get('./userInfo').subscribe(data => expect(data).toEqual({ success: true }));
    httpMock.expectOne(baseUrl + '/userInfo').flush({ success: true });
